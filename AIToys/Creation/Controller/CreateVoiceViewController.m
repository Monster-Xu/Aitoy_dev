...
- (void)handleSuccessVoiceAudio:(VoiceModel *)voice {
    ...
    if (voice.sampleAudioUrl && voice.sampleAudioUrl.length > 0) {
        ...
        self.speekLabel.text = @"已有录音，可重新录制";
    }
}
...
- (void)handleEditVoiceSave {
    ...
    switch (voice.cloneStatus) {
        ...
        case VoiceCloneStatusCloning:
            // 克隆中状态不应该允许编辑
            [self showAlert:@"音色正在克隆中，请稍后再试"];
            break;
            
        default:
            NSLog(@"⚠️ 未知音色状态: %ld", (long)voice.cloneStatus);
            [self showAlert:@"音色状态异常，无法保存"];
            break;
    }
}
...
- (void)updateVoiceWithChanges:(NSDictionary *)changes voice:(VoiceModel *)voice {
    ...
    [[AFStoryAPIManager sharedManager] updateVoice:updateRequest 
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            // 显示成功提示并返回
            [self showSuccessAlertWithCompletion:@"音色信息更新成功！"];
        });
...
- (void)updateVoiceWithEditRequest:(NSDictionary *)changes voice:(VoiceModel *)voice {
    ...
    if (!self.isUploading) {
        ...
        [SVProgressHUD showWithStatus:@"更新音色中..."];
    }
...
- (void)updateStoryWithParameters:(NSDictionary *)parameters {
    ...
    // 显示加载提示
    [SVProgressHUD showWithStatus:@"更新故事中..."];
    ...
    [[AFStoryAPIManager sharedManager] updateStory:updateRequest 
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [SVProgressHUD dismiss];
            [self showSuccessAlertWithMessage:@"故事信息更新成功！"];
        });
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (error.code == -1009) {
                ...
                errorMessage = [NSString stringWithFormat:@"故事更新失败: %@", error.localizedDescription];
            }
...
- (void)startVoiceCloning {
    ...
    [[AFStoryAPIManager sharedManager] createVoice:voiceRequest
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            // 显示成功信息，用户点击确定后再跳转
            [self showSuccessAlertWithCompletion:@"声音克隆已启动！\n\n系统正在后台处理您的声音，\n请稍候片刻后刷新查看进度。"];
        });
...
- (void)showExitConfirmationDialog {
    [LGBaseAlertView showAlertWithTitle:@"声音复刻尚未保存，确定要离开吗？"
                                content:nil
                           cancelBtnStr:@"取消"
                          confirmBtnStr:@"离开"
...
- (void)updateRecordingTime {
    ...
    if (self.recordedTime < 30) {
        // 还没达到最少时间要求，提醒用户
        self.speekLabel.text = [NSString stringWithFormat:@"录音中，建议至少30秒，松开结束(%lds)", (long)self.recordedTime];
    } else {
        // 已达到建议时间，正常显示
        self.speekLabel.text = [NSString stringWithFormat:@"录音中，松开结束录音(%lds)", (long)self.recordedTime];
    }
}
